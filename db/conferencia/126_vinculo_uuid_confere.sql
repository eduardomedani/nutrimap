-- ===========================================================================
-- A CONFERENCIA DOS UUID — roda no Supabase, so le
-- ---------------------------------------------------------------------------
-- Os pares (id, nome) abaixo saem do vinculo da primeira leva. A consulta
-- compara cada um com o cadastro e devolve UMA LINHA POR DIVERGENCIA.
--
-- RESULTADO ESPERADO: NENHUMA LINHA. Qualquer linha aqui interrompe o processo
-- antes de qualquer upload — e o motivo mais provavel e erro de transcricao do
-- mapa, nao erro de curadoria.
-- ===========================================================================
...
-- Guarda espelhada: o nome existe, mas aponta para outro id?
with esperado(exercicio_id, nome) as (values
  ('a981ea9f-8fc5-4651-836d-612e436031ea'::uuid, 'Afundo caminhando livre'),
  ('415d29f0-d14b-48a0-b369-82cecb0f9f15'::uuid, 'Barra fixa (pegada pronada)'),
  ('659d8da0-604c-4f79-b785-a831cb76fbf2'::uuid, 'Cadeira adutora'),
