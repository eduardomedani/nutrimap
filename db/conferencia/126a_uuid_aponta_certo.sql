-- ===========================================================================
-- 126a · CADA UUID APONTA PARA O NOME CERTO?
-- ---------------------------------------------------------------------------
-- NAO ALTERA NADA. So le. UMA consulta so — este arquivo existe justamente
-- para que o resultado nao possa ser confundido com o da outra.
--
-- Os pares (id, nome) saem do vinculo da primeira leva. A consulta compara
-- cada um com o cadastro e devolve UMA LINHA POR DIVERGENCIA.
--
-- RESULTADO ESPERADO: NENHUMA LINHA.
--
-- Qualquer linha interrompe o processo antes de qualquer upload. O motivo mais
-- provavel e erro de transcricao do mapa, nao erro de curadoria: o par
-- (id, nome) continua coerente comigo, so nao corresponde ao banco.
-- ===========================================================================
...
select
  e.exercicio_id,
  e.nome                                    as nome_esperado,
  x.nome                                    as nome_no_banco,
  case when x.id is null then 'UUID NAO EXISTE'
       else 'NOME DIFERENTE' end            as problema
from esperado e
left join public.exercicios x on x.id = e.exercicio_id
where x.id is null or x.nome is distinct from e.nome;
