-- ===========================================================================
-- ETAPA 1B - EXTRACAO: public.nutricionistas
-- ---------------------------------------------------------------------------
...
--   db/conferencia/67_legacy_nutricionistas_LIMPO.sql
-- ===========================================================================
...
with colunas as (
  ...
  where a.attrelid = 'public.nutricionistas'::regclass
    and a.attnum > 0
...
constraints as (
  ...
  where c.conrelid = 'public.nutricionistas'::regclass
),
indices as (
  select
    'INDICE' as secao,
    ...
    i.indexdef as linha
  from pg_indexes i
  where i.schemaname = 'public' and i.tablename = 'nutricionistas'
),
rls as (
  select
    'RLS' as secao,
  ...
  from pg_class c
  where c.oid = 'public.nutricionistas'::regclass
),
policies as (
  select
    'POLICY' as secao,
    ...
    p.policyname
      ...
      as linha
  from pg_policies p
  where p.schemaname = 'public' and p.tablename = 'nutricionistas'
),
triggers as (
  select
    'TRIGGER' as secao,
  ...
  from pg_trigger t
  where t.tgrelid = 'public.nutricionistas'::regclass
...
referencias as (
  ...
  from pg_constraint c
  where c.confrelid = 'public.nutricionistas'::regclass
),
tudo as (
  select * from colunas
