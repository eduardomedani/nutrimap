-- ===========================================================================
-- Evollo · A COBRANCA PASSA A SE CHAMAR PELO CLIENTE, E O PLANO VIRA CATEGORIA
-- ---------------------------------------------------------------------------
-- Requer db/comercial_periodo_da_cobranca.sql. 100% re-executavel.
--
-- O QUE MUDA, em uma linha: a cobranca de assinatura deixa de se chamar
-- "Mensal - 5x — Fulano" e passa a se chamar "Fulano", com "Mensal - 5x" na
-- COLUNA CATEGORIA, que ate hoje nascia vazia.
--
-- TRES COISAS SE CONSERTAM DE UMA VEZ:
--
--   1. A LISTA FICA LEGIVEL. A coluna Descricao repetia o plano em toda linha,
--      e o nome do cliente — que e o que se procura — vinha depois de um
--      prefixo que muda de tamanho. Agora o nome comeca na margem.
--
--   2. A CATEGORIA PASSA A EXISTIR. Cobranca de assinatura nascia sem
--      categoria (`categoria_id` default null), entao o relatorio de receita
--      por categoria so enxergava o que veio da planilha. Metade do
--      faturamento ficava fora do grafico.
--
--   3. A GUARDA ANTI-DUPLICATA DA IMPORTACAO VOLTA A FUNCIONAR. Ela compara
--      `lower(trim(descricao)) = lower(trim(nome))`, e era exatamente o
--      prefixo do plano que a fazia errar — foi assim que cinco pagamentos
--      entraram duas vezes em agosto/2026 (conferencia 122). Com a descricao
--      sendo o nome, nome + data + valor voltam a casar e a linha repetida e
--      pulada, em vez de virar dinheiro novo.
--
-- ===========================================================================
-- A CATEGORIA E ENCONTRADA, NAO INVENTADA
-- ---------------------------------------------------------------------------
-- `comercial_categoria_do_plano` procura uma categoria de RECEITA com o nome
-- do plano e so cria se nao houver. Isso importa: a importacao de vendas ja
-- criou "Mensal - 5x", "Mensal - 3x" e "Diaria" a partir da coluna "Pacote" da
-- planilha. Reaproveitar essas mesmas categorias e o que faz o historico
-- importado e as cobrancas novas somarem no MESMO grupo do relatorio — criar
-- uma segunda "Mensal - 5x" racharia o total em duas sem ninguem perceber.
--
-- O `on conflict do nothing` seguido de novo select existe para o caso de duas
-- cobrancas nascerem ao mesmo tempo: `uniq_financeiro_categorias_nome` e
-- (nutri_id, tipo, lower(nome)), entao a segunda perde a corrida e precisa ler
-- o que a primeira gravou, em vez de estourar.
--
-- Assinatura sem plano devolve NULL: sem nome de plano nao ha categoria, e
-- inventar "Mensalidade" criaria um grupo que nao corresponde a contrato
-- nenhum.
--
-- ===========================================================================
-- O QUE ESTE SCRIPT NAO FAZ
-- ---------------------------------------------------------------------------
-- Nao renomeia o que ja esta gravado. As cobrancas antigas continuam
-- "Mensal - 5x — Fulano" e sem categoria — reescrever lancamento pago e mexer
-- em registro financeiro fechado, e isso e decisao separada. O molde do
-- backfill fica em db/financeiro_backfill_rotulo_cobranca.sql, que guarda a
-- descricao velha no `metadata` antes de trocar.
--
-- As DUAS funcoes abaixo sao copia fiel de db/comercial_periodo_da_cobranca.sql
-- com duas linhas trocadas em cada uma: a descricao e a categoria. Nada mais
-- foi tocado — `create or replace` exige o corpo inteiro.
--
-- Rodar no SQL Editor do Supabase.
-- Para colar, use db/comercial_rotulo_da_cobranca_LIMPO.sql
-- ===========================================================================


-- ===========================================================================
-- 1) A categoria do plano — encontra ou cria
-- ===========================================================================
create or replace function public.comercial_categoria_do_plano(
  p_nutri uuid,
  p_plano uuid
)
returns uuid
...
declare
  ...
  v_cat  uuid;
begin
  if p_plano is null or p_nutri is null then return null; end if;

  select pl.nome into v_nome from public.comercial_planos pl where pl.id = p_plano;
  ...
  select c.id into v_cat
   ...
   where c.nutri_id = p_nutri and c.tipo = 'receita'
  ...
  values (p_nutri, v_nome, 'receita', 99)
...
revoke all on function public.comercial_categoria_do_plano(uuid, uuid) from public, anon;
grant execute on function public.comercial_categoria_do_plano(uuid, uuid) to authenticated;


-- ===========================================================================
-- 2) A cobranca do periodo — descricao e categoria novas
-- ===========================================================================
...
-- ===========================================================================
-- CONFERENCIA. Esperado: helper = 1 · prefixo_nas_funcoes = 0
-- ---------------------------------------------------------------------------
-- `prefixo_nas_funcoes` procura o rotulo velho dentro do codigo das duas
-- funcoes. Se vier 1 ou 2, alguma delas nao foi substituida.
-- ===========================================================================
select
  (select count(*) from pg_proc where proname = 'comercial_categoria_do_plano')  as helper,
  (select count(*) from pg_proc
    where proname in ('comercial_criar_cobranca_do_periodo', 'comercial_registrar_pagamento')
      and prosrc like '%Mensalidade%')                                           as prefixo_nas_funcoes;
