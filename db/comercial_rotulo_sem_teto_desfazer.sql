-- ===========================================================================
-- Evollo · DESFAZER — O TETO TEMPORARIO VOLTA
-- ---------------------------------------------------------------------------
-- Desfaz db/comercial_rotulo_sem_teto.sql. GERADO a partir de
-- db/comercial_rotulo_da_cobranca.sql: devolve as duas funcoes ao corpo
-- EXATO do rotulo, com os dois blocos de TETO — o estado de producao em
-- 11/09/2026, antes da correcao.
--
-- EFEITO: volta a valer "so o proprietario da organizacao cria cobranca ou
-- registra pagamento". Qualquer outro login recebe o TETO, que a tela mostra
-- como "Nao foi possivel concluir. Tente novamente."
--
-- GUARDA: so segue se as duas funcoes em producao forem as do rotulo SEM teto
-- — ou seja, se a correcao estiver aplicada e ninguem tiver mexido depois.
--
-- Nao altera dado nenhum.
--
-- Rodar no SQL Editor do Supabase.
-- Para colar, use db/comercial_rotulo_sem_teto_desfazer_LIMPO.sql
-- ===========================================================================

-- ===========================================================================
-- 0) A GUARDA — so segue se producao for o rotulo SEM teto
-- ===========================================================================
do $guarda$
declare
  v_n int;
begin
  ...
  if v_n <> 2 then
    raise exception 'As duas funcoes em producao nao estao no estado esperado (rotulo SEM teto): % de 2. Nada foi alterado. Rode a conferencia do fim deste arquivo e me mostre.', v_n;
  end if;
end $guarda$;

-- ===========================================================================
-- 1) AS DUAS FUNCOES — de volta ao corpo do rotulo, COM teto
-- ===========================================================================
...
-- ===========================================================================
-- CONFERENCIA. Esperado: funcoes 2 · com_teto 2 · com_organizacao 2 ·
